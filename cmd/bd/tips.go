...
func maybeShowTip(store storage.LocalMetadataStore) {
...
func selectNextTip(store storage.LocalMetadataStore) *Tip {
	if store == nil {
		return nil
	}
...
func getLastShown(store storage.LocalMetadataStore, tipID string) time.Time {
...
func recordTipShown(store storage.LocalMetadataStore, tipID string) {
	if store == nil || tipID == "" {
		return
	}
