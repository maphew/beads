...
Examples:
  ...
  bd doctor --migration=pre --json  # Machine-parseable migration validation`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		targetServerMode := doctorTargetUsesServerMode(absPath)
		if isEmbeddedMode() && !targetServerMode && !doctorServer {
...
func doctorTargetUsesServerMode(path string) bool {
	cfg, err := configfile.Load(doctor.ResolveBeadsDirForRepo(path))
